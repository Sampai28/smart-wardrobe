"""
Outfit recommender - generates and ranks top-N outfit combinations from the wardrobe.
"""
...
def _filter_by_gender(items: list, gender_filter: str) -> list:
    """Keep items matching the selected gender or tagged Unisex."""
...
def get_top_outfits(conn, compat_weight: float = 0.5, event_weight: float = 0.5,
    ...
    if event_model is not None:
        ...
        for i in range(n):
            t_score = compute_event_score({cls: float(p) for cls, p in zip(USAGE_CLASSES, top_probs[i])}, event)
            b_score = compute_event_score({cls: float(p) for cls, p in zip(USAGE_CLASSES, bottom_probs[i])}, event)
            s_score = compute_event_score({cls: float(p) for cls, p in zip(USAGE_CLASSES, shoes_probs[i])}, event)
